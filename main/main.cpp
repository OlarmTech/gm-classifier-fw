...
extern "C" void app_main(void)
{
    ...
    if (model->version() != TFLITE_SCHEMA_VERSION) {
        TF_LITE_REPORT_ERROR(error_reporter, "Model provided is schema version %d not equal to supported "
    ...
    static uint64_t current_time = esp_timer_get_time() / 1000;
    ...
    while (1) {
        ...
        current_state = get_current_state(voltage[0][0], 1500);
        
        // timeout
        if (esp_timer_get_time() / 1000 - current_time >= kStateTimeout){
            state_buffer[state_counts] = 2500.0/1000.0;
            // feature_buffer[9-state_counts] = 2500.0/1000.0;
            current_time = esp_timer_get_time() / 1000;
            // feature_buffer[10] = (float) current_state;
        ...
        // state change
            ...
            state_buffer[state_counts] = (esp_timer_get_time() / 1000 - current_time) / 1000.0;
            current_time = esp_timer_get_time() / 1000;
        ...
        if (state_counts >= kStateCount || timeout){
            ...
            if (
                ...
                {
                    TF_LITE_REPORT_ERROR(error_reporter, "The results for recognition should contain %d elements, but there are %d in an %d-dimensional shape",kCategoryCount, output->dims->data[1],output->dims->size);
                    return;
                }
            if (output->type != kTfLiteFloat32) {
                TF_LITE_REPORT_ERROR(error_reporter, "The results for recognition should be Float32 elements, but are %d",output->type);
                return;
            }
...
void print_output(float* result) {
    ...
    for (int i = 0; i < kCategoryCount; i++)
    {
        ESP_LOGI(TAG,"%s: %.4f", kCategoryLabels[i],result[i]);
    }
    ESP_LOGI(TAG, "===================");
}
